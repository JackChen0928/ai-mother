...
import com.yupi.yuaicodemother.model.dto.deploy.AppDeployRequest;
...
public class AppController {
     ...
     * 通过SSE流式返回AI生成的代码内容
     *
     * @param appId 应用ID，不能为空且必须大于等于0
     * @param message 用户输入的消息内容，不能为空
     * @param request HTTP请求对象，用于获取当前登录用户信息
     * @return Flux<ServerSentEvent<String>> SSE事件流，包含包装后的JSON数据
     */
    ...
    public Flux<ServerSentEvent<String>> chatGenCode(@RequestParam Long appId, @RequestParam String message, HttpServletRequest request) {
        // 参数校验：检查应用ID和用户消息是否合法
        ThrowUtils.throwIf(appId == null||appId < 0, ErrorCode.PARAMS_ERROR,"应用Id不能未空");
        ThrowUtils.throwIf(StrUtil.isBlank(message), ErrorCode.PARAMS_ERROR,"用户消息不能未空");
